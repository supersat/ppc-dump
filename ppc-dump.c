...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (ftdi_usb_open_desc(ftdi, 0x0403, 0x6014, "C232HM-DDHSL-0", "FTWH8RBI") < 0) {
    ...
    // Run PPC for a while to populate RAM
    /*
    ftdi_write_data(ftdi, "\x80\x10\x13", 3);
    usleep(50000);
    ftdi_write_data(ftdi, "\x80\x00\x13", 3);
    usleep(50000);
    ftdi_write_data(ftdi, "\x80\x10\x13", 3);
    sleep(20);
    */
    ...
    for (i = 0; i < 4; ) {
        ...
        ftdi_write_data(ftdi, "\x80\x11\x13", 3);
        fprintf(stderr, "Resetting PPC... ");
        usleep(50000);
        ftdi_write_data(ftdi, "\x80\x01\x13", 3);
        usleep(50000);
        ftdi_write_data(ftdi, "\x80\x11\x13", 3);
        usleep(50000);
        ...
        */
        ...
        // lis r3, 0
        ftdi_write_data(ftdi, "\x11\x04\x00\x04\x3c\x60\x00\x00", 8);
        // lis r4, 0xdead
        ftdi_write_data(ftdi, "\x11\x04\x00\x04\x3c\x80\xde\xad", 8);
        // stw r4, 0(r3)
        ftdi_write_data(ftdi, "\x11\x04\x00\x04\x90\x83\x00\x00", 8);
